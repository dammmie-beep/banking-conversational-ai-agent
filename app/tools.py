...
def _ok(payload: Any) -> str:
    """Return a JSON success payload (string)."""
    return json.dumps(payload, default=str)


def _err(message: str, extra: Optional[Dict[str, Any]] = None) -> str:
    """Return a JSON error payload (string)."""
...
def execute_tool(tool_name: str, args: dict) -> str:
    ...
    This makes downstream agent parsing reliable.
    """
    try:
        if tool_name == "get_customer_info":
            ...
            # Exclude sensitive keys if present
            ...
            # Assumes data_loader.get_transactions returns newest first (ORDER BY date DESC)
            ...
            # Expecting a dict like {"success": True/False, ...}
            # Still return as-is (JSON).
            return _ok(result)

        return _err("Unknown tool.", {"tool_name": tool_name})

    except Exception as e:
        # Keep errors JSON so the agent can handle them safely
